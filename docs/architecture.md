...
### 開発用のデータ（`supabase/dev/`）

| ファイル | 何をするか |
|---------|-----------|
| `dummy_data.sql` | アカウント・生徒・講師・今月と来月ぶんを丸ごと作り直す。**先に流すのはこれ** |
| `seed_next_month_open.sql` | dummy_data の後に流す。**来月ぶんを空にして「これから提出する」状態にする**（今月は確定済みのまま） |
| `dummy_data_cleanup.sql` | ダミーを消す。**本物のデータを入れる前に必ず流す** |
| `tests/*.sql` | DB の制約・トリガー・RLS の検査。全体が `begin 〜 rollback` の中で走る |

**`seed_next_month_open.sql` を分けてあるのは、dummy_data が来月ぶんの希望と仮確定まで作ってしまうため。** 「保護者が出す → 講師が出す → 管理者が組んで確定する」の入口が最初から埋まっていると、その流れを最初から試せない。消すのは**来月ぶんだけ**で、今月の出欠・授業記録・月謝・給与には触らない。

```bash
node scripts/sql.mjs supabase/dev/dummy_data.sql
