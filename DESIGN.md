...
colors:
  ...
  signature-coral: "#a83a3d"
  signature-forest: "#1a5c34"
...
### Brand & Accent
- **Primary** (`{colors.primary}` — #1a2336): The dominant brand color — a very dark navy. Used for the primary CTA background, the active sidebar item, the "today" marker, and the `{colors.surface-dark}` band. It is not link-blue and not pure black: the slight navy cast is what separates the rtech application surface from a default near-black template.
- **Primary Active** (`{colors.primary-active}` — #121a29): The press state on primary buttons.
...
- **Surface Dark** (`{colors.surface-dark}` — #1a2336): The dark navy surface — mid-page CTA cards on marketing pages, and locked/confirmed badges inside the application.
...
### Text
- **Ink** (`{colors.ink}` — #181d26): The strongest text — h1/h2 display type, table headings, and primary button text-on-light. Deliberately **not** the same hex as `{colors.primary}` (#1a2336): ink is a neutral dark for type, primary is a navy-cast dark for surfaces. Keep them separate — using `{colors.primary}` as a body text color makes long tables read cold and slightly blue.
...
- **Coral** (`{colors.signature-coral}` — #a83a3d): A muted brick red. Full-bleed signature cards with white type, and inside the application it carries the alert/attention role — unread-count badges, danger-tinted secondary buttons, Sunday column headers. Contrast against white is 6.3:1 (AA for normal text).
- **Forest** (`{colors.signature-forest}` — #1a5c34): A mid-tone deep green. Signature cards, and inside the application it is the second categorical color — paired against `{colors.signature-coral}` to distinguish the two businesses/streams at a glance. Contrast against white is 8.0:1.

> **Why these two are mid-tone, not near-black.** The originals sampled from marketing (#aa2d00 / #0a2e0e) were tuned for 48px full-bleed cards. Inside the application these same colors carry 11px text on ~20px-tall chips (calendar events, badges), where a near-black green becomes indistinguishable from `{colors.ink}`. The values here are the application-tuned pair and are the ones actually shipped in `mock/index.html`.
...
- Active state: `button-primary-active` darkens to `{colors.primary-active}` (#121a29).
...
**`signature-coral-card`** — The large full-bleed coral card. Background `{colors.signature-coral}` (#a83a3d, a muted brick red), text `{colors.on-primary}`, rounded `{rounded.lg}` (12px), internal padding `{spacing.xxl}` (48px). Carries an h2 in `{typography.display-md}`, supporting copy in `{typography.body-md}`, and `{component.button-secondary-on-dark}` as the CTA.

**`signature-forest-card`** — A deep green signature card (`{colors.signature-forest}` — #1a5c34) used as a sibling to the coral card.

**`hero-card-dark`** — The dark navy mid-page CTA card. Background `{colors.surface-dark}` (#1a2336), text `{colors.on-dark}`, rounded `{rounded.lg}` (12px), internal padding `{spacing.xxl}` (48px). The same hex as `{colors.primary}` — the system uses one navy-dark for both the primary action and the signature dark surface.
...
### Don't
- Don't make `{colors.link}` (#1b61c9) the primary button color. It is the link color. The primary button is `{colors.primary}` (#1a2336, dark navy). Treating link-blue as the brand action is the most common mistake when reading the source CSS variables.
